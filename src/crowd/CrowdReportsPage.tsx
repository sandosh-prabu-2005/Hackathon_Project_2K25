...
import { CROWD_API_BASE } from "../config/apiBase";
...
export default function CrowdReportsPage() {
  ...
  const [crowdRoutesAvailable, setCrowdRoutesAvailable] = useState<boolean>(false);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (!capabilities.hasCrowdReports || !capabilities.hasCrowdReportsWs) {
        ...
        if (!capabilities.hasCrowdReports) {
          setReports([]);
          setTotal(0);
          setError("Crowd reports API is not available in the currently deployed backend.");
        }
      }
    })();
    return () => {
  ...
  }, [crowdRoutesAvailable, filters, userLocation, scopeMode, radiusKm]);
  ...
  }, [crowdRoutesAvailable, wsConnected, loadData]);
  ...
  }, [crowdRoutesAvailable, loadData, wsDisabled]);
